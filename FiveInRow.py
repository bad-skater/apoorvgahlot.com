...
class FiveInRowServer:
	...
	def placePiece(self, x, y, ID):
		...
		#vertical down
		for i in range(1,5):
			if (x + i) > 14 or self.board[x+i][y] != ID:
				break
			else:
				highest += 1
		#vertical up
		for i in range(-4,0):
			...
			if (x + i < 0) or self.board[x+i][y] != ID:
				break
			else:
				highest += 1
		if highest >= 5:
		...
		# horizontal cases
		for i in range(1, 5):
			if (y+i) > 14 or self.board[x][y+i] != ID:
				break
			else:
				highest += 1

		for i in range(-4,0):
			if (y+i) < 0 or self.board[x][y+i] != ID:
				break
			else:
				highest += 1
		...
		#diagonal cases
		for i in range(1, 5):
			if (x+i) > 14 or (y + i) > 14 or self.board[x+i][y+i] != ID:
				break
			else:
				highest += 1
		for i in range(-4, 0):
			if (x + i) < 0 or (y + i) < 0 or self.board[x+i][y+i] != ID:
				break
			else:
				highest += 1
		if highest >= 5:
		...
		highest = 1
		for i in range(1, 5):
			if (x +i) > 14 or (y-i) < 0 or self.board[x+i][y-i] != ID:
				break
			else:
				highest += 1

		for i in range(-4, 0):
			if (x + i) < 0 or (y - i) > 14 or self.board[x+i][y-i] != ID:
				break
			else:
				highest += 1
